...
const handleSimulate = async (rowId: number, input: string) => {
    ...
    if (input.length === 0) {
      ...
      // Clear Previous Animation completely
      setNodes((nds) => nds.map((n) => ({
        ...n, data: { ...n.data, isHighlighting: false, isHighlighted: false }
      })))

      setEdges((dgs) => dgs.map((d) => ({
        ...d,
        data: { ...d.data, isAnimating: false },
      ...
      for (const [edgePath, targetId] of path) {
        ...
        // 1. SMART EDGE SEARCHING (Handles "q0-a,b-q1")
        const [querySrc, queryLabel, queryTgt] = edgePath.split('-');
        const edgeToAnimate = displayEdges.find(e => {
            if (e.id === edgePath) return true;
            const [eSrc, eLabels, eTgt] = e.id.split('-');
            if (eSrc === querySrc && eTgt === queryTgt) {
                return eLabels.split(',').includes(queryLabel);
            }
            return false;
        });
        
        const animId = Date.now(); // 2. UNIQUE KEY for React to force animation
        ...
        if (edgeToAnimate) {
          setEdges((dgs) => dgs.map((e) => 
            e.id === edgeToAnimate.id
              ? {
                ...e,
                data: {...e.data, isAnimating: animId}, // Pass timestamp
        ...
        await sleep(800); // 3. WAIT LONGER (Matches 0.8s dur)

        // Update Nodes
        setNodes((nds) => nds.map((n) => {
          if (n.id === targetId) return { ...n, data: { ...n.data, isHighlighting: true } }
          if (n.id === sourceId) return { ...n, data: { ...n.data, isHighlighting: false, isHighlighted: true } }
          return n
        }));
          ...
          setEdges((dgs) => dgs.map((e) => 
            e.id === edgeToAnimate.id 
              ? { 
                  ...
                  style: {...e.style, stroke: '#3b82f6', strokeWidth: 3}
                }
              : e
          ))
        }
